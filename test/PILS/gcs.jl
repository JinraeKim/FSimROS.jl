...
@pydef mutable struct ControlNode <: rosNode.Node
	...
	self.i = 0
		...
		# u_cmd = (m*g/6)*ones(6)  # TODO: change it
		df = DataFrame()
		...
		if self.i % 1000 == 0
			self.get_logger().info("control: $(u_cmd)")
		end
		self.i = self.i + 1
...
function main(args=nothing)
     ...
     rclpy.shutdown()
    #pos_cmd_func = t -> [0, 0, 0]
    #env = ControllerEnv(pos_cmd_func)
    #x0 = State(env.multicopter)()  # dummy initial state of multicopter
    #simulator = Simulator(State(env)(), Dynamics!(env), x0; tf=10_000)  # second
    #@show simulator.integrator.p
    #@show simulator.integrator.u
    #step_until!(simulator, 0.001)
    #@show simulator.integrator.u
    #nothing
end
